// content.js (Rewritten to extract structured data)
...
(async () => {
    ...
    const extractTranscriptData = async () => {
        try {
            // STEP 1 & 2: Expand description and open transcript panel
            ...
            if (descriptionBox) {
                ...
                if (expandButton) { expandButton.click(); await new Promise(resolve => setTimeout(resolve, 300)); }
            }
            ...
            if (!transcriptPanel || transcriptPanel.offsetParent === null) {
                ...
                if (directTranscriptButton) { directTranscriptButton.click(); transcriptButtonClicked = true; }
                if (!transcriptButtonClicked) {
                    ...
                    if (moreActionsButton) {
                        ...
                        const buttonInMenu = await waitForElement('ytd-menu-service-item-renderer a, ytd-menu-service-item-renderer yt-button-shape');
                        if (buttonInMenu && buttonInMenu.innerText.toLowerCase().includes("transcript")) { buttonInMenu.click(); transcriptButtonClicked = true; }
                    }
                }
                if (!transcriptButtonClicked) return { error: "'Show transcript' button not found." };
            }

            // STEP 3: Wait for transcript to render and get content
            const transcriptRenderer = await waitForElement('ytd-transcript-renderer', 5000);
            if (!transcriptRenderer) return { error: "Transcript panel did not open." };
            ...
            // STEP 4: Extract all items into a structured array
            ...
            if (transcriptItems.length === 0) return { error: "Could not find any transcript content." };
